 ...
 */
 ...
 define([
    ...
    'jquery',
    'uiComponent',
    'FCheckout_OneStepCheckout/js/model/agreements-modal',
    ...
    return Component.extend({
        defaults: {
            template: 'FCheckout_OneStepCheckout/checkout/checkout-agreements'
        },
        ...
        placeOrder: function () {
            $("#checkout .messages").attr('id', 'MensagemErro');
            ...
            if (!this.notValidShipping()) {
                if (this.validateForm('#checkout_agreements_block')) {
                    if ($('#submit-shipping-method').length) {//if not virtual product
                        ...
                        let is_rm_pagseguro_cc = $('input#rm_pagseguro_cc').length;
                        this.setBillingForced();
                        if ($('input#moipcc').length || is_rm_pagseguro_cc) {
        ...
        notValidShipping: function () {
            ...
            $('div#shipping-new-address-form ._required input').each(function () {
                var removeInput = ["region"];
                if ($('select[name="drop_down"]').val() == 1) {
                    removeInput = ["region", "inscricao_estadual", "razao_social"];
                }

                if (!removeInput.includes($(this).attr('name'))) {
                    if ($(this).val() == '') {
                        if (count == 0) {
                            $("#" + $(this).attr('id')).get(0).scrollIntoView();
                        }

                        $(this).css('border-color', '#ed8380');
                        $(this).val('1');
                        $(this).trigger('keyup');
                        $(this).val('');
                        $(this).trigger('keyup');
            ...
            $('.checkout-shipping-address input').each(function () {
                var addInput = [];
                if ($('input[name="code-password"]').val()) {
                    addInput = ["code-confirm_password"];
                }

                if (addInput.includes($(this).attr('name'))) {
                    if ($(this).val() == '' || $('input[name="code-password"]').val() != $('input[name="code-confirm_password"]').val()) {
                        if (count == 0) {
                            $("#" + $(this).attr('id')).get(0).scrollIntoView();
                        }
