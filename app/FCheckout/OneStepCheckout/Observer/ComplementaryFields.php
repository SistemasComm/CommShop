...
namespace FCheckout\OneStepCheckout\Observer;
...
class ComplementaryFields implements ObserverInterface
{
    ...
    protected function convertGuestToCustomer($order, $data)
    {
        ...
        if (isset($data->mm_dob)) {
            $newDate = explode('/', $data->mm_dob);
            if (isset($newDate[0]) && isset($newDate[1]) && isset($newDate[2])) {
                $customer->setDob($newDate[0] . '-' . $newDate[1] . '-' . $newDate[2]);
            }
        }
        ...
        if (isset($data->inscricao_estadual)) {
            $customer->setInscricaoEstadual($data->inscricao_estadual);
        }

        if (isset($data->razao_social)) {
            $customer->setRazaoSocial($data->razao_social);
        }
