    ...
    \Magento\Shipping\Model\Carrier\CarrierInterface
{
    ...
    public function collectRates(RateRequest $request)
    {
        ...
        if (!$this->isActive() || strlen($request->getData('dest_postcode')) < 8) {
            return false;
        }
        ...
        foreach ($modalidade_frete as $correiosMethod) {
            $correiosMethod = $this->getServiceToPopulate($correiosMethod, $this->formatZip($request->getData('dest_postcode')));
    ...
    public function getServiceToPopulate($service, $finalPostcode)
    {
        ...
        if ((bool)$this->getConfig('carriers/correios/owner_hands') === false) {
            $ownerHands = 'N';
        } else {
            $ownerHands = 'S';
        }
        if ((bool)$this->getConfig('carriers/correios/received_warning') === false) {
            $receivedWarning = 'N';
        } else {
            $receivedWarning = 'S';
        }
        ...
        if (in_array($service, $this->obligatoryLogin) === true && ($login === "" || $password === "")) {
            $this->logMessage("Impossível calcular o serviço " . $service . " porque o login / senha não está preenchido.");
            return false;
        }
        if ($login != "") {
            $url_d = $url . "&nCdEmpresa=" . $login . "&sDsSenha=" . $password . "&nCdFormato=1&nCdServico=" .
                $service . "&nVlComprimento=" . $this->_midSize . "&nVlAltura=" . $this->_midSize . "&nVlLargura=" .
                $this->_midSize . "&sCepOrigem=" . $origPostcode . "&sCdMaoPropria=" . $ownerHands . "&sCdAvisoRecebimento=" .
                $receivedWarning . "&nVlPeso=" . $packageWeightMiniEnvio . "&sCepDestino=" . $finalPostcode;
        } else {
            $url_d = $url . "&nCdFormato=1&nCdServico=" . $service . "&nVlComprimento=" . $this->_midSize . "&nVlAltura=" .
            $this->_midSize . "&nVlLargura=" . $this->_midSize . "&sCepOrigem=" . $origPostcode . "&sCdMaoPropria=" .
                $ownerHands . "&sCdAvisoRecebimento=" . $receivedWarning . "&nVlPeso=" . $packageWeightMiniEnvio . "&sCepDestino=" . $finalPostcode;
        }
        if ($declaredValue) {
            ...
            $url_d = $url_d . "&nVlValorDeclarado=$sub_total";
        }
    ...
    public function getOfflineShippingQuotes($service, $finalPostcode, $urlsArray)
    {
        ...
        foreach($cotacaoOfflineObj as $cotacaoOffline){
            if($cotacaoOffline['servico'] == $service){
                ...
                $final_value = $cotacaoOffline['valor'];
                if($this->_splitUp){
                    $final_value *= 2; 
                }
                $data['valor'] = str_replace(",", ".", $final_value) + $handlingFee + $additionalValue;
    ...
    public function getOnlineShippingQuotes($service, $finalPostcode, $urlsArray)
    {
        ...
        foreach ($urlsArray as $url_d) {
            ...
            try {
                ...
                curl_setopt($ch, CURLOPT_TIMEOUT, $timeOutCur);
                curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, $timeOutCur);
                ob_start();
                curl_exec($ch);
                curl_close($ch);
                ...
                foreach ($xml->cServico as $servico) {
                    if ((float)$servico->Valor > 0) {
                        try {
                            $data = [];
                            if (!$showDeliveryDays) {
                                $data['servico'] = $this->getMethodName($servico->Codigo).$debug_mode;
                            } else {
                                $data['servico'] = sprintf(
                                        $deliveryMessage,
                                        $this->getMethodName($servico->Codigo),
                                        intval($servico->PrazoEntrega + $this->_delivery_day)
                                    ).$debug_mode;
                            }

                            $final_value = $servico->Valor;
                            if($this->_splitUp){
                                $final_value *= 2; 
                            }

                            $data['valor'] = str_replace(",", ".", $final_value) + $handlingFee + $additionalValue;
                            $data['prazo'] = $servico->PrazoEntrega + $this->_delivery_day;
                            $data['servico_codigo'] = (string)$servico->Codigo;
                            array_push($ratingsCollection, $data);
                            if ($servico->MsgErro != "") {
                                $this->logMessage($servico->MsgErro);
                            }
                        } catch (\Exception $ex) {
    ...
    public static function getTrackingInfoByCode($obj){
        if($obj){
            ...
            curl_setopt_array($curl, array(
              CURLOPT_URL => 'https://correios.contrateumdev.com.br/api/rastreio',
