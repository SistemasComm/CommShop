...
use MestreMage\SigepWeb\PhpSigep\Model\CalcPrecoPrazo as calculoPrazopreco;
use MestreMage\SigepWeb\PhpSigep\Model\CalcPrecoPrazoResposta;
use MestreMage\SigepWeb\PhpSigep\Model\ServicoDePostagem;
use MestreMage\SigepWeb\PhpSigep\Model\CalcPrecoPrazoRespostaIterator;
...
class CalcPrecoPrazo
{
     ...
     */
    public function execute(calculoPrazopreco $params)
    {
        $larguraMinima     = 0;
        $alturaMinima      = 0;
        ...
        if (is_object($r) && property_exists($r, 'CalcPrecoPrazoResult') && is_object($r->CalcPrecoPrazoResult)
            ...
            if ($r->CalcPrecoPrazoResult->Servicos->cServico) {
                ...
                foreach ($servicos as $servico) {
                    ...
                    $item = new CalcPrecoPrazoResposta(array(
                        'servico'               => new ServicoDePostagem($servico->Codigo),
                        'valor'                 => $valor,
        ...
        $retornoIterator = new CalcPrecoPrazoRespostaIterator($retorno);
