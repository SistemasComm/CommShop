...
use MestreMage\SigepWeb\PhpSigep\Pdf\Chancela\Pac2018;
...
class CartaoDePostagem2018
{
    ...
    public function __construct($plp, $idPlpCorreios, $logoFile, $chancelas = array())
    {
        ...
        if ($logoFile && !@getimagesize($logoFile)) {
            $logoFile = dirname(__FILE__).'/logo-etiqueta-2016.png';
        }
        ...
        while (count($objetosPostais)) {
            ...
            switch ($servicoDePostagem->getCodigo()) {
                ...
                case ServicoDePostagem::SERVICE_SEDEX_CONTRATO_AGENCIA_TA:
                    $simbolo_de_encaminhamento = dirname(__FILE__).'/simbolo-sedex-standard.png';
                    $_texto = 'SEDEX';
                    break;
                case ServicoDePostagem::SERVICE_SEDEX_12:
                    $simbolo_de_encaminhamento = dirname(__FILE__).'/simbolo-sedex-expresso.png';
                    $_texto = 'SEDEX 12';
                    break;
                ...
                case ServicoDePostagem::SERVICE_CARTA_COM_A_FATURAR_SELO_E_SE:
                    $simbolo_de_encaminhamento = dirname(__FILE__).'/simbolo-sem-especificacao.png';
                    $_texto = 'Carta';
                    break;
            ...
            $hEtiquetaBarCode = 18;
            $wEtiquetaBarCode = 80;
            $code128 = new \MestreMage\SigepWeb\PhpSigep\Pdf\Script\BarCode128();
            $code128->draw(
                $this->pdf,
                5,
                $tPosEtiquetaBarCode,
    ...
    {
        $this->pdf = new ImprovedFPDF('P', 'mm', array(106.36, 140));
    ...
    private function writeDestinatario ($l, $t, $w, $objetoPostal)
    {
        ...
        $cep = '';
        $destino = $objetoPostal->getDestino();
        if ($destino instanceof \MestreMage\SigepWeb\PhpSigep\Model\DestinoNacional) {
    ...
    private function writeRemetente ($l, $t, $w, \MestreMage\SigepWeb\PhpSigep\Model\Remetente $remetente)
    {
        $titulo = 'Remetente:';
    ...
    private function writeEndereco (
        ...
        if ($destinatario === true) {
            ...
            $this->t($w, $titulo, 2, '');
            $this->pdf->SetTextColor(0,0,0);
            $this->pdf->Image(dirname(__FILE__).'/correios-logo.png', 84, $t+1, 20, 4);
