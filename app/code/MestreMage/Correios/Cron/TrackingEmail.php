...
class TrackingEmail
{
    public function execute()
    {
        ...
        foreach ($orderDatamodel as $order) {
            ...
            if ($trackNumbers) {
                ...
                if (isset($tracking['success']) && isset($tracking[0]['date']) && isset($tracking[0]['hour'])) {
                    ...
                    if ($order->getPayment()->getAdditionalInformation('object_time_tracking') != $change_tracking) {
                        ...
                        $this->setStatusCompleteShipping($order, $tracking);
                    }
                }
            }
        }
    ...
    public function setStatusCompleteShipping($order, $tracking)
    {
        if (isset($tracking[0]['message'])) {
            if (preg_match("/Objeto entregue/i", $tracking[0]['message'])) {
                $order->setStatus('entregue_correios')->save();
            }
        }
    }
}
